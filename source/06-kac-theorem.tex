Сначала приведем несколько вспомогательных утверждений и
  ...
  \proof{
    \begin{trivlist}
      \item а)
        ...
        По лемме~\ref{Lie(H) - superideal} в силу нормальности $ G' $ получаем,
        что $ \forall ~i ~\Lie(G^{(i)}) $ --- суперидеал, а по лемме~\ref{subgroups iff Lie subalgebras}
        имеем $ \forall ~i ~\Lie(G^{(i)}) \subseteq \Lie(G^{(i+1)}) $.
